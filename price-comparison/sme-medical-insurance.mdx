...
- `payment_link`: where the company pays, once `pending_declarations` is empty. It is a short link that opens Yasmina's secure card page, where the company pays for all of its employees in one go, so it can be sent in a message as it is. The same link comes back on every read; it is valid for 24 hours and is replaced with a new one as it nears expiry. It is null while anyone still has to declare, and again once the policy is issued.

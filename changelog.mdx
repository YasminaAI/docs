...
<Update label="14th September 2026" tags={["Docs", "APIs"]}>
	Rewrote the [Errors](/api-resources/errors) page: the error envelope, every error code with what to do about it, and the errors each step of the motor and SME medical journeys can return. Every API error is now JSON with a `code`, whatever the `Accept` header says. Business-rule rejections (`42203`, for example a payment link for a company with nothing left to pay) and rate limits (`42901`, with `retry_after_seconds`) used to come back as an HTML page, or as a bare `message` without a code. `POST /api/v1/car-comp/issue-otp` now answers `400` instead of `200` when the quote request belongs to another client. An unreachable insurer is reported as `504` / `50401`, not `502`.
</Update>

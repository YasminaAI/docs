...
Call it **before you request the payment OTP**: the insurance provider checks every
image during this call, so a photo it cannot read is rejected while the customer can
still retake it.

<Warning>This step is required for comprehensive cover. If the photos are not uploaded,
the customer cannot continue with the purchase.</Warning>
...
- **JPG or PNG, up to 20 MB each**, and 100 MB for the five together.
- **Once per vehicle.** A second upload comes back as a `409`.
- **Comprehensive only.** The insurance provider rejects a `quote_price_id` belonging to a TPL quote.

...
4. **Enrol Employees**: Put the employees who will be covered onto the company's policy with the chosen provider.
...
### Enrol the employees
A company buys one policy, and its employees are the members on it, which is how the insurer issues it and how the policy document reads. Call the [Enrol employees API](/medical-api-reference/policies/enrol-employees) and you get that one policy back, with every employee inside `meta_data.insured` and the premium for all of them in `price`.

Enrol again before the policy is paid for and the same policy is updated: an employee already on it, matched by their id, is replaced, and anyone new is added. Once it is paid for and issued, the next enrolment starts a new policy.

...
The <a href="/medical-api-reference/company/create-company" target="_blank">The Create Company API</a>, will require a few fields that define details of the company such as Email, Phone, the 10-digit sponsor number and commercial registration number, and the unified number.
...
Call it once per provider, passing `insurance_provider=medgulf` or `insurance_provider=bupa` (it defaults to `medgulf`). Each call gives you that provider's insurance categories.

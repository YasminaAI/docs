...
Call it **before you request the payment OTP**. The insurance provider downloads and
checks every image during this call, so a photo it cannot read is rejected while the
customer can still retake it — rather than after they have paid.
...
- **JPG or PNG, up to 20 MB each**, and 100 MB for the five together. Other formats are rejected.
...
- **Comprehensive only.** The insurance provider accepts photos on comprehensive cover only, and rejects a `quote_price_id` belonging to a TPL quote.

...
<Steps>
  ...
  <Step title="Open a session">
    Call **Create Embed Session** with the same payload you would send to [Request Quotes](/car-api-reference/quotes/request-quotes), plus where the session will run. We fetch the quotes and hand back a URL.
  </Step>
  <Step title="Show it">
...
`allow="camera"` matters only if you want customers to take vehicle photos with their camera rather than choosing existing ones.
...
Add `NSCameraUsageDescription` and `NSPhotoLibraryUsageDescription` to your `Info.plist`, or the app closes when the customer opens the photo picker.
...
`GET /api/v1/car-comp/embed-sessions/{id}` returns a session you already created, and tells you whether it has expired.
...
<Tip>
  Send `Accept: application/json` on every call. Without it an error comes back as an HTML page instead of a JSON body.
</Tip>
